 ...
 * @param {String} prefix Prefix to apply to the parameter name
 *
...
function getNode(code, lineNum) {
  ...
  /**
   * correctLine - Is the node on the correct line to be affected by this jsdoc
   *
   * @param {int} line The line number to check
   *
   * @returns {bool} Whether the line passed is the correct line
   */
  function correctLine(line) {
    return (line === lineNum || line === (lineNum + 1));
  }

  walker.simple(ast, {
    FunctionDeclaration: (n) => {
      const funcLine = n.loc.start.line;
      if (correctLine(funcLine)) {
        node = n;
      }
    },
    VariableDeclaration: (n) => {
      const declarationLine = n.loc.start.line;
      const declarator = n.declarations[0];
      if (correctLine(declarationLine) && declarator.type === 'VariableDeclarator') {
