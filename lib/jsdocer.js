  ...
  const cleanParams = params
    .reduce((col, param) => {
      if (param.type === 'Identifier') {
        col.push({ name: param.name, type: DEFAULT_PARAM_TYPE });
        return col;
      } else if (param.type === 'AssignmentPattern') {
        col.push({ name: `[${param.left.name}=${param.right.value}]`, type: DEFAULT_PARAM_TYPE });
        return col;
      } else if (param.type === 'ObjectPattern') {
        col.push({ name: DEFAULT_PARAM_NAME, type: 'Object' });
        const props = param.properties;
        for (let i = 0; i < props.length; ++i) {
          const prop = props[i];
          col.push({ name: `${DEFAULT_PARAM_NAME}.${prop.key.name}`, type: DEFAULT_PARAM_TYPE });
        }
        return col;
      }
