...
/**
 * extractParameterNameProps - Extract additional parameter properties from the
 * name
 *
 * @param {string} rawName Raw parameter name
 *
 * @return {object} Properties of the parameter that can be extracted from the name
 */
function extractParameterNameProps(rawName) {
  const res = { name: rawName };
  if (rawName.includes('.')) {
  ...
  if (rawName.startsWith('[') && rawName.endsWith(']')) {
    const result = /\[(.*)\=(.*)\]/.exec(rawName);
    if (!result || result.length !== 3) {
      throw new Error('Invalid parameter with default value');
    }
    res.name = result[1];
    res.defaultValue = result[2];
  }
  return res;
}
...
function paramParser(content) {
  const parts = content.split(/\s+/);
  const type = extractType(parts[1]);
  const nameProps = extractParameterNameProps(parts[2]);
  const description = parts.slice(3).join(' ');
  return Object.assign({}, nameProps, {
    type,
    description,
  });
}
