 ...
 */
export function render({ name, description = 'Description', params = [],
  returns = {}, location = {} }) {
  const open = '/**';
  const spacer = ' *';
  ...
  const lines = header.concat(renderedParams).concat(footer);

  const { column = 0 } = location;
  const indentation = linePadder.repeat(column);
