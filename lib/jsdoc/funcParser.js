...
function parseAssignmentParam(param) {
  ...
  if (paramAssignmentType === 'StringLiteral') {
    ...
    type = 'null';
    defaultValue = null;
  } else if (paramAssignmentType === 'ArrowFunctionExpression') {
    type = 'function';
    defaultValue = '() => {}';
    ...
    type = 'Unknown';
    defaultValue = 'Unknown';
  } else if (paramAssignmentType === 'MemberExpression') {
    type = 'Unknown';
    defaultValue = 'Unknown';
  } else {
 ...
 */
function parseDestructuredParam({ properties }) {
  ...
  return props.reduce(
    (params, param) => {
      ...
      if (param.type === 'ObjectProperty') {
        value = param.value; // eslint-disable-line prefer-destructuring
      }
 ...
 */
export default function parse(code, lineNum = 1) {
